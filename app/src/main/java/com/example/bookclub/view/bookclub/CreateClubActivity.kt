        ...
        binding.confirmTV.setOnClickListener {
            ...
            when {
                clubName=="" -> {   //클럽명이 적혀 있지 않을 때
                    Toast.makeText(this@CreateClubActivity, "클럽명을 입력해주세요", Toast.LENGTH_SHORT).show()
                }
                clubInfo=="" -> {   //클럽 한줄 소개가 적혀 있지 않을 때
                    Toast.makeText(this@CreateClubActivity, "클럽 한줄 소개를 입력해주세요", Toast.LENGTH_SHORT).show()
                }
                else -> {   //클럽명 생성 요청을 서버에 전달한다.
                    var newClub: ClubModel = ClubModel(name=clubName, description=clubInfo, colorSet=colorSet)
                    ...
                    CoroutineScope(Dispatchers.Main).launch {
                        ...
                        when (res.await().code()) {
                            201 -> {    //CreateClubContract 에 새로 생성된 club 데이터를 전달한다.
                                ...
                                setResult(Activity.RESULT_OK, Intent().apply { putExtra("newClub", bundle) })

                                finish()
                            }
                            400 -> {    //중복 클럽명이 존재하는 상황
                                Toast.makeText(this@CreateClubActivity, "이미 사용 중인 클럽명입니다.", Toast.LENGTH_SHORT).show()
                            }
                        }
                    }
                }
